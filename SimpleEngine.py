...
class Sudoku:
    def __init__(self, game, not_built=True, z={}):
        self.arr = game
        self.zero = z
        if not_built:
    ...
    def is_input_valid(self):
        for i in range(9):
            for j in range(9):
                node = self.arr[i][j]
                if node:
                    if not isinstance(node, int) or not 1 <= node <= 9:
                        return False
                    for a, b in depend[(i+1, j+1)]:
                        if node == self.arr[a-1][b-1]:
                            return False
        return True
    ...
    def update(self, i, j):
        if not self.arr[i][j]:
            a, b = (i // 3) * 3, (j // 3) * 3
            self.zero[(i + 1, j + 1)] = set(range(1, 10)) - set(
                self.arr[i]
                + [self.arr[x][j] for x in range(9)]
                + [self.arr[a + x][b + y] for x in (0, 1, 2) for y in (0, 1, 2)]
            )
